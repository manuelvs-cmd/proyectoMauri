...
@section('title', 'Editar Pedido - Múltiples Mercancías')
    ...
    <div class="row justify-content-center">
        <div class="col-md-10">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('pedidos.update', $pedido) }}" method="POST" id="pedido-form">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group">
                            ...
                            <div class="add-mercancia-btn" onclick="agregarMercancia()">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="date" name="fecha_entrega" id="fecha_entrega" class="form-control" value="{{ old('fecha_entrega', $pedido->fecha_entrega) }}" required>
                                    @error('fecha_entrega')
                                        <div class="alert alert-danger mt-2">{{ $message }}</div>
                                    @enderror
                                </div>
...
let pedidoMercancias = @json($pedido->mercancias);
...
function agregarMercancia(mercanciaExistente = null) {
    mercanciaCounter++;
        ...
        <div class="col-md-3">
            <div class="form-group">
                ...
                <input type="number" name="mercancias[${mercanciaCounter}][precio_unitario]" 
                       ...
                       data-index="${mercanciaCounter}" value="${mercanciaExistente ? (mercanciaExistente.pivot.precio_unitario || '') : ''}">
    ...
    mercanciaDiv.innerHTML = `
        <button type="button" class="remove-mercancia" onclick="removerMercancia(${mercanciaCounter})" title="Eliminar mercancía">
                ...
                <div class="form-group">
                    ...
                    <div class="position-relative">
                        <input type="text" class="form-control mercancia-search" placeholder="Buscar mercancía..." 
                               data-index="${mercanciaCounter}" autocomplete="off" value="${mercanciaExistente ? mercanciaExistente.nombre : ''}">
                        <div class="dropdown-search" id="dropdown-${mercanciaCounter}" style="display: none;"></div>
                        <input type="hidden" name="mercancias[${mercanciaCounter}][mercancia_id]" class="mercancia-id" value="${mercanciaExistente ? mercanciaExistente.id : ''}">
                    </div>
                    <small class="form-text text-muted mercancia-info">${mercanciaExistente ? `${mercanciaExistente.nombre} - $${new Intl.NumberFormat('es-CL').format(mercanciaExistente.precio_venta)}` : 'Selecciona una mercancía'}</small>
                </div>
            </div>
            <div class="col-md-3">
                <div class="form-group">
                    ...
                    <input type="number" name="mercancias[${mercanciaCounter}][cantidad_solicitada]" 
                           class="form-control cantidad-input" min="1" value="${mercanciaExistente ? mercanciaExistente.pivot.cantidad_solicitada : '1'}" required 
                           data-index="${mercanciaCounter}">
                    <small class="form-text text-muted stock-info">Stock: ${mercanciaExistente ? mercanciaExistente.cantidad : '-'}</small>
                </div>
            </div>
            ${precioColumn}
        </div>
        ...
        <div class="row mt-2">
            <div class="col-12">
                <div class="alert alert-info mercancia-selected" style="${mercanciaExistente ? 'display: block;' : 'display: none;'}">
                    <strong>Seleccionado:</strong> <span class="mercancia-nombre">${mercanciaExistente ? mercanciaExistente.nombre : '-'}</span>
    ...
    // Si es una mercancía existente, marcarla como activa y actualizar subtotal
    if (mercanciaExistente) {
        mercanciaDiv.classList.add('active');
        actualizarSubtotal(mercanciaCounter);
    }
...
// Cargar mercancías existentes del pedido al cargar la página
document.addEventListener('DOMContentLoaded', function() {
    // Cargar las mercancías existentes del pedido
    pedidoMercancias.forEach(mercancia => {
        agregarMercancia(mercancia);
    });
    
    // Si no hay mercancías, agregar una vacía
    if (pedidoMercancias.length === 0) {
        agregarMercancia();
    }
});
</script>
